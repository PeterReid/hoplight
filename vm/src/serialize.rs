...
impl Serializer {
    ...
    fn serialize_atom(&mut self, atom_bytes: &[u8]) -> SerializationResult<()> {
        ...
        if atom_bytes.len() >= self.maximum_atom_encoding_length
            || self.atom_encoding.len() >= self.maximum_atom_encoding_length - atom_bytes.len()
        {
            return Err(SerializationError::MaximumLengthExceeded);
        }
    ...
    fn serialize_noun(&mut self, noun: &Noun) -> SerializationResult<()> {
        match noun.as_kind() {
            NounKind::Cell(lhs, rhs) => {
                ...
                self.serialize_noun(lhs)?;
                self.serialize_noun(rhs)?;
            }
            NounKind::Atom(bytes) => {
                self.structure.push(false);
                self.serialize_atom(bytes)?;
            }
        }
        Ok(())
    }
...
pub fn serialize(noun: &Noun, maximum_atom_encoding_length: usize) -> SerializationResult<Vec<u8>> {
    ...
    serializer.serialize_noun(noun)?;

    let length_encoding = {
        let mut length_encoder = Serializer::new(10);
        length_encoder.serialize_noun(&Noun::from_usize_compact(serializer.atom_encoding.len()))?;
...
mod test {
    ...
    fn serialize_medium_atom() {
        assert_eq!(
            serialize(&(&[9, 8, 7, 6, 5, 4, 3, 2, 1, 0][..]).as_noun(), 100),
    ...
    fn serialize_little_trees() {
        assert_eq!(
            serialize(&((40, 50), 60).as_noun(), 100),
