...
impl<'a> Deserializer<'a> {
    ...
    fn deserialize_atom(&mut self) -> DeserializeResult<Noun> {
        let kind = self.consume_byte()?;

        match kind {
            literal if literal < 190 => Ok(Noun::from_u8(literal)),
            x => {
                let length = if x != 255 {
                    x as usize - 190
                } else {
    ...
    fn deserialize_noun(&mut self) -> DeserializeResult<Noun> {
        let is_cell = self.consume_structure_bit()?;
        if is_cell {
            let left = self.deserialize_noun()?;
            let right = self.deserialize_noun()?;
    ...
    fn check_exhausted(&mut self) -> DeserializeResult<()> {
        ...
        if self.structure_buffer.len() > 1
            || (self.structure_buffer.len() == 1 && self.structure_bit_pos == 0)
        {
            return Err(DeserializeError::UnexpectedContinuationOfStream);
        }
...
pub fn deserialize(buf: &[u8]) -> DeserializeResult<Noun> {
    ...
    let length = match d.deserialize_atom()?.as_usize() {
        Some(length) => length,
        None => {
...
mod test {
    ...
    fn a_few_bytes_atom() {
        assert_eq!(
            deserialize(&[5, 194, 254, 253, 252, 251, 0][..]),
            Ok((&[254, 253, 252, 251][..]).as_noun())
        );
    }
